...
# ---------------------------------------------------------------------
# figure defaults (AGU-friendly aspect and DPI; no grids)
# ---------------------------------------------------------------------
...
def plot_hucs(
    ...
    """plot huc2 polygons with state boundaries; CONUS by default."""
...
def build_site_series_from_json(
    ...
    """
    traverse json {huc -> {site_no -> info}}; extract time series by site.
    prefers 'series_key' first, then any in 'alt_keys'.
    returns {site_no: series}
    """
    out = {}
...
def load_target_series(
    ...
    """
    fetch daily values from NWIS for a single site and normalize.
    """
