...
## CSTM Figures
def plot_model_results(results, forecast_horizons, *, dpi=600):
    """
    Plot training and evaluation metrics across forecast horizons.

    Parameters
    ----------
    results : dict
        Nested dictionary with structure results[horizon][metric] -> (n_runs, n_epochs).
    forecast_horizons : list
        Forecast horizons (e.g., [1, 3, 5, 7]).
    dpi : int
        Figure resolution.
    ...
    colors = [cm.cm.haline(0.05), cm.cm.haline(0.2), cm.cm.haline(0.4),
              cm.cm.haline(0.55), cm.cm.haline(0.65), cm.cm.haline(0.75)]
    linestyles = ['--', '-.', ':', '--', '-.', ':']

    fig, axs = plt.subplots(1, 3, figsize=(7, 2), sharex=True, sharey=True, dpi=dpi)
...
def plot_timeseries(preds_dict, forecast_horizons, key_true, key_pred, key_dates,
                    date_start, date_end, *, dpi=600, ylim=(0, 1_750_000)):
    """
    Plot measured and predicted discharge time series across forecast horizons.

    Parameters
    ----------
    preds_dict : dict
        Dictionary with preds_dict[horizon][0] containing keys [key_true, key_pred, key_dates].
    forecast_horizons : list
        Horizons to plot.
    key_true : str
        Key for observed values.
    key_pred : str
        Key for predicted values.
    key_dates : str
        Key for datetime values.
    ...
    fig, ax = plt.subplots(figsize=(7, 2), dpi=dpi)
    ...
    for idx, horizon in enumerate(forecast_horizons):
        ...
        ax.plot(dates[mask], y_pred[mask], label=f"{horizon} Day",
