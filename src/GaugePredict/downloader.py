...
def _rank_parameter_col(name, preference):
    ...
    **Inputs** :
        ...
        preference : 'list of str'
            ...
            Earlier entries are preferred (lower rank).
...
def _pick_parameter_col(df, parameter_code, *, parameter_kind=None):
    ...
    **Inputs** :
        ...
        parameter_kind : 'str or None'
            Optional hint to tune ranking. Expected values include
...
def _as_utc_daily_index(full_index, *, tz="UTC"):
    ...
    This helper is intended for a user-provided "full_index" defining desired
...
def _convert_units_from_parameter_code(ts, parameter_code, *, to_units="metric"):
    ...
    **Inputs** :
        ...
        to_units : {'metric', 'native', None}
            Target unit system. Only "metric" is supported for conversion.
...
def load_target(
    ...
    *,
    ...
    small set of common parameter codes.
...
def GaugebyHUC(
    ...
    *,
    ...
    Notes:
    - Daily timestamps are handled in UTC for consistency across platforms.
    - `data_dir` is created for compatibility with workflows that expect an output
      directory, even though this function only writes the JSON file.
